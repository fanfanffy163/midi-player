...
class IndexBuilderTask(QRunnable):
    ...
    def run(self):
        try:
            ...
            print(f"Whoosh: 正在提交 {total_count} 个文件...")
            writer.commit()
            print("Whoosh: 索引构建完毕。")
            ...
            try:
                ...
                print(f"警告：索引 .meta 文件写入失败: {meta_e}")
            
            self.signals.index_ready.emit(True) 
            
        except Exception as e:
            print(f"创建 Whoosh 索引失败: {e}")
...
class MidiCards(QWidget):
    ...
    def _generate_stylesheet(self,color) -> str:
        primary_color = ThemeColor.PRIMARY.color().name() if color == None else color.name()
        return f"""
    ...
    def load_index_and_directory(self, dir_path_str: str, force_rebuild: bool = False):
        ...
        if needs_rebuild:
            print(f"需要重建索引。原因: {rebuild_reason}")
            self.is_index_ready = False
    ...
    def on_search_results(self, filtered_paths: list):
        ...
        print(f"正在渲染 {len(paths_to_display)} 个卡片，并异步派发加载任务...")
            ...
            if not card:
                try:
                    ...
                    if target_page != self.current_page:
                        ...
                        self.on_search_results(self.current_filtered_paths)
                        # 渲染后高亮 (需要延迟)
                        QTimer.singleShot(50, lambda: self.on_card_clicked(None, str(next_path)))
                except ValueError:
                    pass
