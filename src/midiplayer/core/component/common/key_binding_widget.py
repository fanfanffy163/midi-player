...
class KeyCaptureLineEdit(LineEdit):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        key = event.key()
        if (
            self.max_key_count is not None
            and len(self.pressed_keys) > self.max_key_count - 1
        ):
            return
    ...
    def format_key_combo(self) -> str:
        ...
        modifiers.sort(
            key=lambda m: (
                [
                    QT_MODIFIER_KEYS[Qt.Key.Key_Control],
                    QT_MODIFIER_KEYS[Qt.Key.Key_Shift],
                    QT_MODIFIER_KEYS[Qt.Key.Key_Alt],
                    QT_MODIFIER_KEYS[Qt.Key.Key_Meta],
                ].index(m)
                if m in QT_MODIFIER_KEYS.values()
                else 99
            )
        )
    ...
    def show_placeholder_text(self):
        if self.max_key_count is None:
            ...
            self.setPlaceholderText(
                f"点击并按下按键组合,最多支持{self.max_key_count}个按键"
            )
