...
def NoteFitting(tracks : list[MidiTrack], note_to_key_mapping : dict[str,str], disableNoteFitting : bool) -> tuple[dict[str,str], float, int]:
    ...
    # 转化mapping为int
    mapped_note_to_keys_mapping  : dict[int,str] = {MIDI_NOTE_MAP.get_midi_by_note(k):v for k,v in note_to_key_mapping.items()}   
    ...
    while mapped_max_note > min_note and mapped_max_note >= 0:
        ...
        mapped_min_note -= 12
        mapped_max_note -= 12
        tmp_mapped_note_to_keys_mapping = {key - 12: value for key, value in tmp_mapped_note_to_keys_mapping.items()}
    ...
    while finest_plan != 0:
        if finest_plan > 0:
            finest_plan -= 1
            mapped_note_to_keys_mapping = {key + 12: value for key, value in mapped_note_to_keys_mapping.items()}
        else:
            finest_plan += 1
            mapped_note_to_keys_mapping = {key - 12: value for key, value in mapped_note_to_keys_mapping.items()}
    note_to_key_mapping = {MIDI_NOTE_MAP.get_note_by_midi(key) : value for key,value in mapped_note_to_keys_mapping.items() if key >= 0 and key <= 127}   
