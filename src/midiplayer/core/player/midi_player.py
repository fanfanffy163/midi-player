...
from .type import MdPlaybackParam,MIDI_NOTE_MAP,KEY_MAP,CONTROL_KEY_MAP
...
class QMidiPlayer(QtCore.QObject):
    ...
    def prepare(self, md_playback_param: MdPlaybackParam):
        ...
        with self.clock_lock:
            ...
            tracks = self._find_main_track(self.midi) if player_play_single_track else self.midi.tracks
            self.note_to_key, correct_radio_1base,octave_change  = NoteFitting(tracks,self.note_to_key,cfg.get(cfg.player_play_disable_note_fitting))
            for track in tracks:
                current_tick = 0
                for msg in track:
